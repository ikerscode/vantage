...
**Update (BRIEF v1.6)**: this has now actually been run for real, for the first time, in CI — see `OFFLINE_BUNDLE_REPORT.md` for the genuine measured tarball size (~6.6 GiB), the resulting shipping-strategy decision (chunked separate download, not embedded in the installer — the real number turned out to be over 3x GitHub's per-file release-asset cap), and the real clean-machine/network-cut acceptance test this section's "not run here" gap had been blocking.
